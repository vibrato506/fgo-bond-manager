...
def generate_markdown(result_a, result_b, result_c, config, title, timber_boxes):
    ...
    if timber_boxes > 0:
        md.append("---\n")
        md.append("## 🪵 【参考】副産物の獲得予測\n")
        md.append(f"すでに収集は完了済みですが、全プランの中で最も周回数が少ない「最短{min_runs}周」を基準として、副産物として集まるアイテムの量を予測しておきます。\n")
        md.append("**【現在の所持数】**（画像からの指定値）")
        md.append("*   **副産物A**: 50 個")
        md.append("*   **副産物B**: 127 個")
        md.append("*   **副産物C**: 68 個\n")
        md.append("> [!TIP]")
        md.append(f"> 1周につき副産物が平均 **{timber_boxes}枠** 落ちると仮定した場合、最短{min_runs}周で得られる総枠数は **各{frames:.1f}枠** となります。\n")
        md.append("最終的な獲得量は、**自由枠にどのクラスのサーヴァントを編成するか**によって大きく変動します。\n")
        md.append("### 獲得量の計算式")
        ...
        md.append("- **[基本編成パターン1] の場合（ボーナス: A+4, B+1, C+3）**")
        md.append(f"   *   副産物A: 50 + (7個 × {frames:.1f}枠) = **{int(50 + 7*frames):,} 個**")
        md.append(f"   *   副産物B: 127 + (4個 × {frames:.1f}枠) = **{int(127 + 4*frames):,} 個**")
        md.append(f"   *   副産物C: 68 + (6個 × {frames:.1f}枠) = **{int(68 + 6*frames):,} 個**\n")
        md.append("- **[基本編成パターン2] の場合（ボーナス: A+3, B+3, C+1）**")
        md.append(f"   *   副産物A: 50 + (6個 × {frames:.1f}枠) = **{int(50 + 6*frames):,} 個**")
        md.append(f"   *   副産物B: 127 + (6個 × {frames:.1f}枠) = **{int(127 + 6*frames):,} 個**")
        md.append(f"   *   副産物C: 68 + (4個 × {frames:.1f}枠) = **{int(68 + 4*frames):,} 個**\n")
        md.append("- **[特効マシマシ編成] の場合（ボーナス: A+5, B+5, C+3）**")
        md.append(f"   *   副産物A: 50 + (8個 × {frames:.1f}枠) = **{int(50 + 8*frames):,} 個**")
        md.append(f"   *   副産物B: 127 + (8個 × {frames:.1f}枠) = **{int(127 + 8*frames):,} 個**")
        md.append(f"   *   副産物C: 68 + (6個 × {frames:.1f}枠) = **{int(68 + 6*frames):,} 個**\n")
        md.append("このように、もし特定の副産物に偏らせて集めたい場合は、目的を意識して編成することで獲得量を大幅に増やすことが可能です！")
...
def generate_short_markdown(result_a, result_b, result_c, config, title, timber_boxes):
    ...
    if timber_boxes > 0:
        ...
        frames = min_runs * timber_boxes
        md.append("### 🪵 副産物予測")
        md.append(f"- **[基本編成パターン1]** -> A:{int(50 + 7*frames):,} / B:{int(127 + 4*frames):,} / C:{int(68 + 6*frames):,}")
        md.append(f"- **[基本編成パターン2]** -> A:{int(50 + 6*frames):,} / B:{int(127 + 6*frames):,} / C:{int(68 + 4*frames):,}")
        md.append(f"- **[特効マシマシ編成]** -> A:{int(50 + 8*frames):,} / B:{int(127 + 8*frames):,} / C:{int(68 + 6*frames):,}\n")
...
def get_quest_results(quest_key, quest_data, config_template):
    ...
    timber_boxes = quest_data.get('timber_boxes', 0)
    ...
    long_md = generate_markdown(result_a, result_b, result_c, config, quest_data['title'], timber_boxes)
    short_md = generate_short_markdown(result_a, result_b, result_c, config, quest_data['title'], timber_boxes)
