...
class FixedLoadoutSolver:
    ...
    def _solve_for_friend(self, fixed_friend_type, friend_evolved, targets):
        ...
        while iteration < sanity_limit:
            ...
            target_inventory = {k: {'normal': 0, 'evolved': 0} for k in ['gold', 'silver', 'copper']}
            # Generic assumption for base drops
            base_drops = {k: self.materials[k].get('boxes', 10) * self.BASE_DROP for k in ['gold', 'silver', 'copper']}
                ...
                if req_B > 0:
                    ...
                    while curr_B < req_B:
                        ...
                        if remaining == 1 and avail_n > 0:
                            avail_n -= 1; sn += 1; curr_B += 1
                        elif avail_e > 0:
                            avail_e -= 1; se += 1; curr_B += 2
                        elif avail_n > 0:
                            avail_n -= 1; sn += 1; curr_B += 1
                        else: break
...
def generate_markdown(result_a, result_b, result_c, config, title, timber_boxes):
    ...
    md.append("| :--- | :--- | :--- |")
    md.append(f"| 🟡 **雀の幸（金素材）** | {config['materials']['gold']['owned']:,} 個 | {config['materials']['gold']['needed']:,} 個 |")
    md.append(f"| ⚪️ **川の幸（銀素材）** | {config['materials']['silver']['owned']:,} 個 | {config['materials']['silver']['needed']:,} 個 |")
    md.append(f"| 🟠 **山の幸（銅素材）** | {config['materials']['copper']['owned']:,} 個 | {config['materials']['copper']['needed']:,} 個 |\n")
    ...
    def format_phases(phases):
        ...
        for i, phase in enumerate(phases):
            ...
            if phase.get('completed_resource') and not is_last:
                comp = []
                if 'gold' in phase['completed_resource']: comp.append("🟡 **雀の幸（金素材）**")
                if 'silver' in phase['completed_resource']: comp.append("⚪️ **川の幸（銀素材）**")
                if 'copper' in phase['completed_resource']: comp.append("🟠 **山の幸（銅素材）**")
                if comp:
                    lines.append("> [!NOTE]")
    ...
    def format_final(res):
        ...
        lines = []
        for key, icon, name in [('gold', '🟡', '雀の幸（金素材）'), ('silver', '⚪️', '川の幸（銀素材）'), ('copper', '🟠', '山の幸（銅素材）')]:
    ...
    frames = min_runs * timber_boxes
    md.append("---\n")
    md.append("## 🪵 【参考】木材の獲得予測（副産物）\n")
    md.append(f"すでに改築・木材収集は完了済みですが、全プランの中で最も周回数が少ない「最短{min_runs}周（プランA・Cなど）」を基準として、副産物として集まる木材の量を予測しておきます。\n")
    md.append("**【現在の所持数】**（画像からの指定値）")
    md.append("*   **ケヤキ**: 50 個")
    md.append("*   **ヒノキ**: 127 個")
    md.append("*   **スギ**: 68 個\n")
    md.append("> [!TIP]")
    md.append(f"> 1周につき木材が平均 **{timber_boxes}枠** 落ちると仮定した場合、最短{min_runs}周で得られる総枠数は **各{frames:.1f}枠** となります。\n")
    md.append("最終的な木材の獲得量は、**自由枠にどのクラスのサーヴァントを編成するか**によって大きく変動します。\n")
    md.append("### 獲得量の計算式")
    md.append("*   **1枠あたりの獲得数** ＝ 基礎(3個) ＋ クラスボーナス合計")
    md.append(f"*   **最終所持数** ＝ 現在の所持数 ＋ (1枠あたりの獲得数 × {frames:.1f}枠)\n")
    md.append(f"**【編成ごとの最終結果シミュレーション (最短{min_runs}周の場合)】**")
    md.append("編成の空き枠に、どのサーヴァントを置くかでの違いです。\n")
    md.append("- **ご指定の編成 [全+1鯖:1、ケヤキ:3、ヒノキ:0、スギ:2] の場合（ボーナス: ケヤキ+4, ヒノキ+1, スギ+3）**")
    md.append(f"   *   ケヤキ: 50 + (7個 × {frames:.1f}枠) = **{int(50 + 7*frames):,} 個**")
    md.append(f"   *   ヒノキ: 127 + (4個 × {frames:.1f}枠) = **{int(127 + 4*frames):,} 個**")
    md.append(f"   *   スギ: 68 + (6個 × {frames:.1f}枠) = **{int(68 + 6*frames):,} 個**\n")
    md.append("- **ご指定の編成 [全+1鯖:1、ケヤキ:2、ヒノキ:2、スギ:0] の場合（ボーナス: ケヤキ+3, ヒノキ+3, スギ+1）**")
    md.append(f"   *   ケヤキ: 50 + (6個 × {frames:.1f}枠) = **{int(50 + 6*frames):,} 個**")
    md.append(f"   *   ヒノキ: 127 + (6個 × {frames:.1f}枠) = **{int(127 + 6*frames):,} 個**")
    md.append(f"   *   スギ: 68 + (4個 × {frames:.1f}枠) = **{int(68 + 4*frames):,} 個**\n")
    md.append("- **ご指定の編成 [全+2鯖:1、全+1鯖:1、ケヤキ:2、ヒノキ:2、スギ:0] の場合（ボーナス: ケヤキ+5, ヒノキ+5, スギ+3）**")
    md.append("   *(※いただいた条件の「全+1鯖：2、全+1鯖：1」はパーティ上限(6枠)の都合上、「全+2鯖：1、全+1鯖：1」の誤記と推測して計算しています)*")
    md.append(f"   *   ケヤキ: 50 + (8個 × {frames:.1f}枠) = **{int(50 + 8*frames):,} 個**")
    md.append(f"   *   ヒノキ: 127 + (8個 × {frames:.1f}枠) = **{int(127 + 8*frames):,} 個**")
    md.append(f"   *   スギ: 68 + (6個 × {frames:.1f}枠) = **{int(68 + 6*frames):,} 個**\n")
    md.append("このように、もし特定の木材に偏らせて集めたい場合は、全+2鯖・全+1鯖や目的クラスを意識して編成することで、副産物の木材を大幅に増やすことが可能です！")
    return "\n".join(md)

def generate_short_markdown(result_a, result_b, result_c, config, title, timber_boxes):
    md = []
    md.append(f"# 📊 {title} (簡潔版)\n")
    
    md.append(f"- 🟡金: {config['materials']['gold']['owned']:,} / {config['materials']['gold']['needed']:,}")
    md.append(f"- ⚪️銀: {config['materials']['silver']['owned']:,} / {config['materials']['silver']['needed']:,}")
    md.append(f"- 🟠銅: {config['materials']['copper']['owned']:,} / {config['materials']['copper']['needed']:,}\n")
    
    plans = [
        ('プランA(最適・フレンド切替)', result_a, result_a['total_runs']),
        ('プランB(金礼装固定)', result_b, result_b['total_runs']),
        ('プランC(自前礼装フリー)', result_c, result_c['total_runs'])
    ]
    # 周回数でソート (同数の場合はA, B, Cの順。Cは編成コストが低いので、同数ならAよりCを上にするなど工夫も可能だが今回は単純に周回数優先)
    plans.sort(key=lambda x: x[2])
    
    for i, (p_name, res, runs) in enumerate(plans[:2]):
        md.append(f"## {i+1}位: {p_name} (全 {runs} 周)")
        ...
        for j, phase in enumerate(phases):
            ...
            own_str = ", ".join(parts) if parts else "なし"
            md.append(f"- 【{phase['runs']}周】自前: {own_str} / フレ: {friend}")
        
        acc = res['phases'][-1]['accumulated']
        md.append(f"  * 最終: 🟡 {int(acc['gold']):,} / ⚪️ {int(acc['silver']):,} / 🟠 {int(acc['copper']):,}\n")
    
    # 木材予測
    min_runs = plans[0][2]
    frames = min_runs * timber_boxes
    md.append("### 🪵 木材予測")
    md.append(f"- **[全+1:1, ケヤキ:3, スギ:2]** -> ケヤキ:{int(50 + 7*frames):,} / ヒノキ:{int(127 + 4*frames):,} / スギ:{int(68 + 6*frames):,}")
    md.append(f"- **[全+1:1, ケヤキ:2, ヒノキ:2]** -> ケヤキ:{int(50 + 6*frames):,} / ヒノキ:{int(127 + 6*frames):,} / スギ:{int(68 + 4*frames):,}")
    md.append(f"- **[全+2:1, 全+1:1, ケヤキ:2, ヒノキ:2]** -> ケヤキ:{int(50 + 8*frames):,} / ヒノキ:{int(127 + 8*frames):,} / スギ:{int(68 + 6*frames):,}\n")
...
def get_quest_results(quest_key, quest_data, config_template):
    ...
    short_md = generate_short_markdown(result_a, result_b, result_c, config, quest_data['title'], quest_data['timber_boxes'])
...
def run_all_quests(config):
    ...
    if not quests:
        ...
        long_md, short_md = get_quest_results('default', q_data, config)
        md_longs.append(long_md)
        md_shorts.append(short_md)
    else:
        for q_key, q_data in quests.items():
            long_md, short_md = get_quest_results(q_key, q_data, config)
            md_longs.append(long_md)
            md_shorts.append(short_md)
