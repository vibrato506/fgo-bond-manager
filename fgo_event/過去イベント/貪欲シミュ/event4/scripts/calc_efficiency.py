...
def generate_all_short_markdown(shorts_data, config):
    ...
    for s_data in shorts_data_sorted:
        title = s_data['title']
        timber_boxes = s_data['timber_boxes']
        min_runs = min(s_data['result_a']['total_runs'], s_data['result_b']['total_runs'], s_data['result_c']['total_runs'])
        frames = min_runs * timber_boxes
        
        lines.append(f"■ {title} (最短 {min_runs} 周基準)")
        lines.append(f"  [全+1:1, ケヤキ:3, スギ:2] -> ケヤキ:{int(50 + 7*frames):,} / ヒノキ:{int(127 + 4*frames):,} / スギ:{int(68 + 6*frames):,}")
        lines.append(f"  [全+1:1, ケヤキ:2, ヒノキ:2] -> ケヤキ:{int(50 + 6*frames):,} / ヒノキ:{int(127 + 6*frames):,} / スギ:{int(68 + 4*frames):,}")
        lines.append(f"  [全+2:1, 全+1:1, ケヤキ:2, ヒノキ:2] -> ケヤキ:{int(50 + 8*frames):,} / ヒノキ:{int(127 + 8*frames):,} / スギ:{int(68 + 6*frames):,}")
        lines.append("")
